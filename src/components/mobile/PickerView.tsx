...
function LiveKpiBanner({ orders = [] }: { orders: Order[] }) {
  ...
  const bales = safeOrders.reduce(
    (sum, order) => sum + (order?.logisticsMetrics?.bellaBags || 0),
    0,
  );
  const pallets = safeOrders.reduce(
    (sum, order) => sum + (order?.logisticsMetrics?.sabanPallets || 0),
    0,
  );
    ...
    <section className="overflow-hidden rounded-2xl border border-sky-500/30 bg-sky-950/20 shadow-sm transition-all">
      ...
      {isOpen && (
        <div className="grid grid-cols-2 gap-2 border-t border-sky-500/20 p-2.5 sm:grid-cols-4 bg-card/40">
          ...
          <div
            className={cn(
              "rounded-xl p-2 border border-border/50 text-center",
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div className="rounded-2xl border border-border bg-card p-2.5 text-center flex flex-col justify-between">
                  ...
                  <span className="text-xl font-black text-foreground my-1">
                    {truckDraft.sandBags}
                  </span>
      ...
      <div className="p-3.5 space-y-2.5">
        ...
        <div className="p-2 rounded-xl bg-secondary/40 border border-border text-[11px] font-bold flex items-center justify-between">
          ...
          <span
            className={cn(
              "font-black",
              isHikmat ? "text-amber-500" : isAli ? "text-primary" : "text-foreground",
            )}
          >
            {isHikmat
