...
type OrderWithLegacyFields = Order & {
  branch?: string;
  stage?: string;
  id?: string;
  client?: string;
...
export function HarashWarehouseApp({
  ...
  const harashOrders = useMemo(() => {
    ...
    return list.filter((o) => {
      const w = String(o?.warehouse || (o as OrderWithLegacyFields)?.branch || "");
  ...
  const pickingOrders = useMemo(() => {
    return harashOrders.filter((o) =>
      ["בסידור עבודה", "בהכנה", "ממתין", "בהמתנה"].some(
        (st) => o?.status?.trim() === st || (o as OrderWithLegacyFields)?.stage?.trim() === st,
      ),
    );
  }, [harashOrders]);
  ...
  useEffect(() => {
    ...
    for (const order of harashOrders) {
      ...
      const itemsText = Array.isArray(order?.items)
        ...
        : "";
      const rawSummary = `${itemsText} ${(order as OrderWithLegacyFields)?.productsSummary || ""} ${(order as OrderWithLegacyFields)?.itemsFormatted || ""}`;
      ...
      if (match) {
        ...
        if (count >= 10) {
          setActiveAlert({
            orderId,
            client: order?.customerName || (order as OrderWithLegacyFields)?.client || "לקוח",
            sumsumCount: count,
          });
          break;
        }
  ...
  const sendBeitHatitWhatsApp = () => {
    const text = [
      ...
      truckDraft.sandSmallPallets > 0
        ? `• ${truckDraft.sandSmallPallets} משטחי שקיות חול (70 שקיות למשטח)`
        : null,
      truckDraft.sumsumSmallPallets > 0
        ? `• ${truckDraft.sumsumSmallPallets} משטחי שקיות סומסום (70 שקיות למשטח)`
        : null,
      ``,
  ...
  return (
    <div
      dir="rtl"
      className="flex flex-col h-screen w-full select-none overflow-hidden bg-background text-foreground font-sans"
    >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="rounded-md bg-amber-500/20 px-1.5 py-0.5 text-[10px] font-black text-amber-500">
                  אורן ותמיר
                </span>
              </div>
          ...
          <div className="flex flex-col gap-3.5">
            ...
            {harashOrders.length === 0 ? (
              ...
              harashOrders.map((order) => {
                const oId = String(order?.orderId || (order as OrderWithLegacyFields)?.id || "");
                return (
                  <HarashOrderCard
                    key={oId}
                    order={order}
          ...
          <div className="flex flex-col gap-4">
              ...
              <div className="mt-3">
                <div className="flex justify-between text-xs font-bold mb-1.5">
                  ...
                  <span
                    className={cn(
                      "font-black",
                      isFullTruck ? "text-emerald-500" : "text-amber-500",
                    )}
                  >
            ...
            <div className="grid grid-cols-3 gap-2.5">
              <div className="rounded-2xl border border-border bg-card p-3 text-center flex flex-col justify-between">
                ...
                <span className="text-2xl font-black text-foreground my-1">
                  {truckDraft.sandBags}
                </span>
                <div className="flex items-center justify-center gap-2">
                  <button
                    type="button"
                    onClick={() =>
                      setTruckDraft((p) => ({ ...p, sandBags: Math.max(0, p.sandBags - 1) }))
                    }
              ...
              <div className="rounded-2xl border border-border bg-card p-3 text-center flex flex-col justify-between">
                ...
                <span className="text-2xl font-black text-foreground my-1">
                  {truckDraft.sumsumBags}
                </span>
                <div className="flex items-center justify-center gap-2">
                  <button
                    type="button"
                    onClick={() =>
                      setTruckDraft((p) => ({ ...p, sumsumBags: Math.max(0, p.sumsumBags - 1) }))
                    }
                    className="size-8 rounded-lg bg-secondary grid place-items-center"
                  >
                    <Minus className="size-4" />
              ...
              <div className="rounded-2xl border border-border bg-card p-3 text-center flex flex-col justify-between">
                ...
                <div className="flex items-center justify-center gap-2">
                  <button
                    type="button"
                    onClick={() =>
                      setTruckDraft((p) => ({ ...p, titBags: Math.max(0, p.titBags - 1) }))
                    }
                    className="size-8 rounded-lg bg-secondary grid place-items-center"
                  >
                    <Minus className="size-4" />
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      onClick={() =>
                        setTruckDraft((p) => ({
                          ...p,
                          sandSmallPallets: Math.max(0, p.sandSmallPallets - 1),
                        }))
                      }
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        setTruckDraft((p) => ({ ...p, sandSmallPallets: p.sandSmallPallets + 1 }))
                      }
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      onClick={() =>
                        setTruckDraft((p) => ({
                          ...p,
                          sumsumSmallPallets: Math.max(0, p.sumsumSmallPallets - 1),
                        }))
                      }
                      className="size-7 rounded-lg bg-secondary grid place-items-center"
                    >
                    ...
                    <button
                      ...
                      onClick={() =>
                        setTruckDraft((p) => ({
                          ...p,
                          sumsumSmallPallets: p.sumsumSmallPallets + 1,
                        }))
                      }
                      className="size-7 rounded-lg bg-primary text-primary-foreground grid place-items-center"
                    >
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <StockCounterCard
                ...
                onDecrement={() =>
                  setFloorStock((p) => ({ ...p, sandBags: Math.max(0, p.sandBags - 1) }))
                }
              />
              <StockCounterCard
                ...
                onDecrement={() =>
                  setFloorStock((p) => ({ ...p, sumsumBags: Math.max(0, p.sumsumBags - 1) }))
                }
              />
              <StockCounterCard
                ...
                onDecrement={() =>
                  setFloorStock((p) => ({ ...p, cementBags: Math.max(0, p.cementBags - 40) }))
                }
              />
              <StockCounterCard
                ...
                onDecrement={() =>
                  setFloorStock((p) => ({ ...p, blocks20: Math.max(0, p.blocks20 - 75) }))
                }
              />
            </div>
          </div>
...
function HarashOrderCard({
  ...
  const orderId = String(order?.orderId || (order as OrderWithLegacyFields)?.id || "");
  const clientName = order?.customerName || (order as OrderWithLegacyFields)?.client || "לקוח כללי";
  const orderTime = order?.targetTime || (order as OrderWithLegacyFields)?.time || "היום";
  const destination = order?.city || (order as OrderWithLegacyFields)?.destination || "איסוף עצמי";

  const productsSummary = useMemo(() => {
    if ((order as OrderWithLegacyFields)?.productsSummary)
      return (order as OrderWithLegacyFields).productsSummary;
    if (Array.isArray(order?.items) && order.items.length > 0) {
      return order.items.map((i) => `${i.quantity} ${i.name}`).join(", ");
    }
    return (order as OrderWithLegacyFields)?.itemsFormatted || "אין פירוט פריטים";
  }, [order]);
    ...
    <div className="overflow-hidden rounded-3xl border border-border/80 bg-card p-4 shadow-sm">
      ...
      <div className="flex items-center justify-between text-[11px] font-bold text-muted-foreground py-1 border-t border-border/50">
        ...
        <button
          type="button"
          onClick={() =>
            onAdvanceStatus(order.status === "מוכן להעמסה" ? "יצא לדרך" : "מוכן להעמסה")
          }
