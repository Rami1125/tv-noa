...
export const GLOBAL_INITIAL_STOCK_BASE = 200;
...
export function calculateLiveInventory(
  ...
  relevantOrders.forEach((order) => {
    ...
    const isCommitted = ["ממתין", "בסידור עבודה", "בסידור מחר", "בהכנה"].includes(order.status);
...
export function calculateDetailedInventoryEngine(
  ...
  relevantOrders.forEach((order) => {
    ...
    const isReserved = ["ממתין", "בסידור עבודה", "בסידור מחר", "בהכנה", "מוכן להעמסה"].includes(
      order.status,
    );
